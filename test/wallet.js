...
contract('Wallet', (accounts) => {
    ...
    it('should have the correct approvers and quorum', async () => {
        ...
        assert(quorum.toNumber() === 2); // using BN.js library, we need to use toNumber()
    });

    it('should creat transfers', async () => {
    ...
    it('should not transfer - wrong approver', async () => {
        await expectRevert(
    ...
    it('should send transfer is quorum reached', async () => {
    ...
    it('should not approve transfer if sender id not approved', async () => {
        await wallet.createTransfer(100, accounts[5], {from: accounts[0]});
        await expectRevert(
